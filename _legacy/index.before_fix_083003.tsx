  ...
  async function runStudio(e?: FormEvent) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        // Fallback mock when product has no /api/tool yet
        setResult(
          PRODUCT.name + ' DEMO\\n\\n' + topic + '\\n\\n---\\nOutcome preview based on your input. Upgrade to Pro for full runs.'
        )
    ...
    <>
      ...
      <div className="min-h-screen bg-white text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <a href="#studio" className="px-6 py-3 rounded-full border border-slate-200 font-bold">▶ Watch Demo</a>
              </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white shadow-xl overflow-hidden">
              <div className="flex gap-2 px-4 py-3 bg-slate-50 border-b border-slate-200">
                <i className="w-3 h-3 rounded-full bg-red-400" />
                <i className="w-3 h-3 rounded-full bg-amber-400" />
                <i className="w-3 h-3 rounded-full bg-emerald-400" />
              </div>
              <div className="p-6 space-y-3">
                <div className="h-3 bg-slate-100 rounded w-4/5" />
                <div className="h-3 bg-slate-100 rounded w-3/5" />
                <div className="h-3 bg-slate-100 rounded w-2/5" />
                <div className="mt-4 text-sm font-semibold text-indigo-600">{PRODUCT.name} · Ready when you are</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-slate-950 text-white py-10">
          <div className="max-w-6xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[['10k+', 'Builders'], ['4.9★', 'Avg rating'], ['99.9%', 'Uptime'], ['<2 min', 'Time to value']].map(([n, l]) => (
              <div key={l}>
                <div className="text-3xl font-black bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent">{n}</div>
        ...
        <section id="features" className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              ...
              <p className="text-slate-600 max-w-2xl mx-auto">Outcome-first features mapped from conversion research: clear hero, interactive demo, transparent pricing, FAQ objections.</p>
            </div>
        ...
        <section id="studio" className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              ...
              <p className="text-slate-600 mt-2">Interactive demo wired to `/api/tool` when available (WriteFlow pattern).</p>
            </div>
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="rounded-2xl border border-slate-200 p-6 bg-white shadow-sm">
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  <button type="button" className="px-4 py-2 rounded-full border border-slate-200 font-semibold" onClick={() => { if (result) { navigator.clipboard?.writeText(result); showToast('Copied') } }}>Copy</button>
                  <a href="/api/checkout" className="px-4 py-2 rounded-full bg-indigo-600 text-white font-semibold">${price}/mo — Upgrade</a>
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="testimonials" className="py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-extrabold">Loved by operators who ship</h2>
            </div>
        ...
        <section id="pricing" className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              <div className="text-xs font-bold tracking-widest uppercase text-indigo-600 mb-3">Pricing</div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="rounded-2xl border border-slate-200 p-6">
                ...
                <button type="button" className="w-full py-3 rounded-full border border-slate-200 font-bold" onClick={() => document.getElementById('signup')?.scrollIntoView({ behavior: 'smooth' })}>Get started free</button>
              </div>
              <div className="rounded-2xl border-2 border-indigo-600 p-6 relative shadow-lg">
                ...
                <div className="text-4xl font-black my-3">${price}<span className="text-base font-semibold text-slate-500">/mo</span></div>
                <ul className="space-y-2 text-sm text-slate-600 mb-6">
                ...
                <a href="/api/checkout" className="block text-center w-full py-3 rounded-full bg-indigo-600 text-white font-bold">Start free trial</a>
                <a href="/api/checkout?cycle=yearly" className="block text-center mt-3 text-sm font-semibold text-indigo-600">Or pay yearly</a>
              </div>
              <div className="rounded-2xl border border-slate-200 p-6">
                ...
                <button type="button" className="w-full py-3 rounded-full border border-slate-200 font-bold" onClick={async () => { await submitLead('sales', 'pricing_contact', 'enterprise'); }}>Contact sales</button>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-20">
          <div id="signup" className="max-w-3xl mx-auto px-6 text-center rounded-3xl bg-slate-950 text-white p-10">
            ...
            <form onSubmit={onSignup} className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <button type="submit" disabled={signupBusy} className="px-6 py-3 rounded-full bg-indigo-500 font-bold disabled:opacity-60">{signupBusy ? 'Saving…' : 'Get started'}</button>
            </form>
                  ...
                  <tbody>
                    {leads.map((l) => (
                      <tr key={l.id} className="border-b border-slate-100">
                        ...
                        <td>{l.source}</td>
                        <td>
                          <button type="button" className="text-red-600 font-semibold" onClick={() => deleteLeadRow(l.id)}>Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {toast ? (
          <div className="fixed bottom-6 right-6 bg-slate-900 text-white px-4 py-3 rounded-xl shadow-lg font-semibold z-50">{toast}</div>
        ) : null}
      </div>
    </>
  )
